...
class File
{
	...
	public function __construct(string $fileName, array $arrTypes)
	{
		...
		{
			throw new FileException('Debes seleccionar un fichero'); //Error		
		}
		...
		if ($this->file['error'] !== UPLOAD_ERR_OK) // Si se ha producido algún error en la subida
		{
			switch ($this->file['error']) {
				...
				case UPLOAD_ERR_FORM_SIZE:
					throw new FileException('El fichero es demasiado grande'); // Error de tamaño
					break;
				case UPLOAD_ERR_PARTIAL:
					throw new FileException('No se ha podido subir el fichero completo'); // Error de archivo incompleto
					break;
				default:
					throw new FileException('No se ha podido subir el fichero'); // Error en la subida
					break;
			}
		}
		...
		if (in_array($this->file['type'], $arrTypes) === false) {
			throw new FileException('Tipo de fichero no soportado'); // Error de tipo
		}
	}
