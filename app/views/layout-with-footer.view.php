<?php
include __DIR__ . '/inicio.part.php';
include __DIR__ . '/navegacion.part.php';
echo $mainContent;
?>
<!-- Footer -->
<div class="ending text-center">
	<ul class="list-inline social-buttons">
		<li>
			<a href="#"><i class="fa fa-facebook sr-icons"></i></a>
		</li>
		<li>
			<a href="#"><i class="fa fa-twitter sr-icons"></i></a>
		</li>
		<li>
			<a href="#"><i class="fa fa-google-plus sr-icons"></i></a>
		</li>
	</ul>
	<ul class="list-inline contact">
		<li class="footer-number">
			<i class="fa fa-phone sr-icons"></i> (00228)92229954
		</li>
		<li>
			<i class="fa fa-envelope sr-icons"></i> kouvenceslas93@gmail.com
		</li>
	</ul>
	<p>Photography Fanatic Template &copy; 2017</p>
</div>
<?php
include __DIR__ . '/fin.part.php';
?>

<!-- Hay que quitar esta parte del código en la página contact.view.php y en el action index de
ContactoController.php cambiamos en el RenderView el parámetro de layout por layoutwith-footer -->